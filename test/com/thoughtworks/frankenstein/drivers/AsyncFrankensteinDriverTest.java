...
/**
 * Ensures the behaviour of the AsyncFrankensteinDriver.
 *
 * @author Pavan
 * @author Prakash
 */
...
//        frankensteinDriver.clickButton(buttonName);
    ...
    public void testCreationOfAsyncFrankensteinDriverObjectDoesAllInitialization() throws ClassNotFoundException {
        String testName = "test";
        Mock testReporterMock = mock(TestReporter.class);
        testReporterMock.expects(once()).method("startTest").with(eq(testName));
        Mock applicationMock = mock(Application.class);
        applicationMock.expects(once()).method("launch").with(ANYTHING);
        new AsyncFrankensteinDriver((Application) applicationMock.proxy(), new String[]{"abc", "def"}, (TestReporter) testReporterMock.proxy(), null, null, null, testName);
    }
