...
public class KeyStrokeMap {
    ...
    static int getModifiersFromText(String modifierText) {
        ...
        for (int i = 0; i < individualModifier.length; i++) {
            Object code = KEY_MODIFIER_MAP.get(individualModifier[i].toUpperCase());
            if (code == null) {
    ...
    static int getKeyCodeFromText(String keyText) {
        String text = keyText.replaceAll(" ", "_");
        Object keyCode = KEY_TEXT_TO_KEY_CODE_MAP.get(text.toUpperCase());
        if (keyCode == null) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (keyName.startsWith("VK_") && !keyName.startsWith("VK_KP_")) {
                try {
                    ...
                    keyTextToKeyCodeMap.put(keyText.toUpperCase(), new Integer(keyCode));
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (field.getName().endsWith("_MASK")) {
                try {
                    ...
                    keyModifierMap.put(KeyEvent.getModifiersExText(modifiers).toUpperCase(), new Integer(modifiers));
                } catch (IllegalAccessException e) {
                }
